 ...
 *
 * The save does not carry team colours — `TeamStore` has 424 members and none
 * of them are decoded — and a hand-written table of 138 schools would be 138
 * chances to be wrong, and useless the moment a dynasty renames a team. The
 * logo is already indexed and is the school's colour by definition, so the
 * colour is read out of it.
 *
 * Only PNG, and only the two forms the game's art actually ships in: 8-bit
 * truecolour with or without alpha, and 8-bit palette. Anything else returns
 * null and the card falls back to the theme, which is the right failure.
 */
export function dominantColor(file: string): string | null {
  ...
  // the paper it is drawn on and mostly a hole.
  ...
  // The most common bucket wins, but a grey one only if there is nothing with
  // colour in it — a white-on-black mark should not come out black.
  const entries = [...bucket.values()].sort((x, y) => y.n - x.n)
  const chroma = (e: { n: number; r: number; g: number; b: number }) => {
    const r = e.r / e.n, g = e.g / e.n, b = e.b / e.n
    return Math.max(r, g, b) - Math.min(r, g, b)
  }
  const total = entries.reduce((s, e) => s + e.n, 0)
  const coloured = entries.find((e) => chroma(e) > 28 && e.n > total * 0.04)
  const win = coloured ?? entries[0]
...
export function decodePng(buf: Buffer): { px: Uint8Array; width: number; height: number } | null {
  ...
  // Undo the per-scanline filter. Every PNG uses these five and a decoder that
  // only handles None reads as noise rather than failing, which is worse.
